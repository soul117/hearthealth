...
public class PresetRadioGroup extends LinearLayout {
    ...
    public void checkWithoutEvent(@IdRes int id) {
        // don't even bother
        if (id != View.NO_ID && (id == mCheckedId)) {
            return;
        }
        if (id != View.NO_ID) {
            setCheckedStateForViewWithoutEvent(id);
        }
    }

    private void clearRadioButtonsSelection() {
        for (Map.Entry<Integer, View> entry : mChildViewsMap.entrySet()) {
            ((RadioCheckable) entry.getValue()).setNormalState();
        }
    }

    private void setCheckedStateForViewWithoutEvent(int viewId) {
        ...
        if (checkedView != null && checkedView instanceof RadioCheckable) {
            ((RadioCheckable) checkedView).setCheckedState();
            mCheckedId = viewId;
        }
    }
